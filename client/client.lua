...
function SetupAnimalPrompt()
	Citizen.CreateThread(function()
		...
		PromptSetControlAction(AnimalPrompt, 0x760A9C6F) -- [G]
...
Citizen.CreateThread(function()
	while true do
		...
		for k, v in pairs(models) do
			if model == v then
				Citizen.InvokeNative(0x081FB9D6422F804C, veh, 5, false) -- SetVehicleDoorCanBreak
				Citizen.InvokeNative(0xA9F1D75195CC40F6, veh, 5, true) -- SetVehicleIndividualDoorsLocked
				Citizen.InvokeNative(0x6A3C24B91FD0EA09, veh, 5, true) -- SetVehicleDoorShut
			end
			Wait(500)
		end
	end
...
Citizen.CreateThread(function()
	...
	while true do
		...
		local animal = Citizen.InvokeNative(0xD806CD2A4F2C2996, ped) -- GetFirstEntityPedIsCarrying
		if animal ~= false then
			...
			local rayCast = StartShapeTestRay(Pos2.x, Pos2.y, Pos2.z, forwardoffset.x, forwardoffset.y, forwardoffset.z, -1, ped,
				7)
			...
			for k, v in pairs(models) do
				if model == v then
					...
					if PromptHasHoldModeCompleted(AnimalPrompt) then
						...
						PromptSetVisible(AnimalPrompt, false)
						prompt = false
						animalcheck = Citizen.InvokeNative(0xD806CD2A4F2C2996, ped) -- GetFirstEntityPedIsCarrying
						pedid = NetworkGetNetworkIdFromEntity(animalcheck)
						Citizen.InvokeNative(0xC7F0B43DCDC57E3D, PlayerPedId(), animalcheck, GetEntityCoords(PlayerPedId()), 10.0, true) -- TaskPlaceCarriedEntityAtCoord
						Wait(1000)
